...
- Never rename DOM ids that JavaScript references (`gameTabs`, `categoryScroll`, `hero`, `homeContent`, `serviceContent`, `serviceGrid`, `detailSection`, `orderForm`, `liveTotal`, `usdHint`, `addToCart`, `cartOpen`, `cartClose`, `cartBackdrop`, `cartBody`, `cartTotal`, `cartUsdHint`, `copyOrder`, `clearCart`, `openDiscord`, `arcIdModal`, `arcIdInput`, `arcIdDone`, `arcIdSkip`, `toast`, `bgMusic`, `audioToggle`, `audioVolume`, `siteSearch`, `siteSearchBtn`, `siteSearchResults`, `currency`, `language`, `brandHome`).
